...
def neighbors_dir(node, directory, neighbor):
    """
    Find all neighbors (leaf nodes) of a node based on a direction
    """
    _neighbors = []
    if neighbor.n_isLeaf:
...
def parent_index(tree, leaf_node):
    ...
    x_width_1st, y_width_1st = (Params.HIGH[0] - Params.LOW[0]) / granularity_1st, (
        Params.HIGH[1] - Params.LOW[1]) / granularity_1st

    i, j = int(((leaf_node.n_box[0][0] + leaf_node.n_box[1][0]) / 2 - leaf_node.n_box[0][0]) / x_width_1st), int(
        ((leaf_node.n_box[0][1] + leaf_node.n_box[1][1]) / 2 - leaf_node.n_box[0][1]) / y_width_1st)
    return i, j
...
def geocast(tree, t, eps):
    ...
    while True:
        ...
        q_log.append((float("%.1f" % c[0]), float("%.3f" % c[1][0]), float("%.3f" % u), float("%.3f" % c[1][2]),
                      float("%.3f" % c[1][3])))
